...
We also load the Denny's and La Quinta datasets.
...
*The Denny's dataset has 1643 observation of 6 variables.   Each observation represents a particular store.  with variables giving the location, both as an address and a lat/long. *
...
*The La Quinta dataset has 909 observations (one for each hotel location), with 6 variables, the same as contained in the Denny's dataset. *
...
_Denny's was only in US, but La Quinta had locations in:_
...
* Check if the "state" is one listed in our `states` dataframe. *
...
```
This check indicates no locations outside US.
...
```
*Most Denny's (per square mile) DC, Rhode Isalnd, California.*
...
lq %>%
  ...
  mutate(per_sq_mile=n/area) %>% arrange(-per_sq_mile)
```
*La Quiant's (per square mile): Rhode Island, Florida, Connecticut.*
...
ggplot(mapping = aes(x = longitude, y = latitude, color = establishment)) +
  geom_point(alpha=.2)+theme_minimal()+ggtitle("Denny's and La Quint in North Carolina")
```
...
ggplot(mapping = aes(x = longitude, y = latitude, color = establishment)) +
  geom_point(alpha=.4)+theme_minimal()+ggtitle("Denny's and La Quint in North Carolina")
```
